---
title: "Hands-on Exercise 10: Modeling Grographical Accessibility"
...
The imported ODMatrix organised the distance matrix columnwise. This means that the column consists of the description of the respective cost going from the origin to the destination.

However, most of the modelling packages in R expects a matrix where the rows represent **origins** (i.e. also know as **from** field) and the columns represent **destination** (i.e. also known as **to** field).
...
We use ac() of SpatialAcc package to compute Hansen's accessibility. It is encourage to read the arguments of the function to ensure that the required inputs are available.
...
You can combine the codes above into one code chunk.
