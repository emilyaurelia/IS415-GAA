...
Learning Note:
-   tmap_mode() with "plot" option -> to produce a static map
...
Disadvantage of qtm():
-   makes aesthetic of individual layers harder to control.
...
-   tm_shape(): to define the input data 
-   (one or more) tm_fill(): 
-   (one or more) tm_polygons(): draw the planning subzone polygons
...
-> to draw a choropleth map showing the geographical distribution os a selected variable by planning subzone
...
Learning Notes:
-   Default interval binning used to draw the choropleth map: "pretty
-   Default colour scheme used: YlOrRd of ColorBrewer
...
-   alpha: transparency between 0 (totally transparent) and 1 (not transparent); by default, the alpha value of the col is used
-   col: border colour
...
breaks argument to customize the breakpoints
--> to have n categories, n+1 elements must be specified in the breaks option (ascending order)
...
tm_compass(): to show compass
tm_scale_bar(): to show scale bar
tm_grid(): to show grid lines for the map
